...
Enriches with codebase context
(dbt models, gusty SQL, schema docs)
...
Launch one subagent per database
using the Task tool:
...
- concept → table mappings (pre-populated from code if found)
Categorical Columns
- value families for key filter columns
Database sections
- one per database
Schema subsections
...
Key column:
**
{PRIMARY_KEY from code or inferred}
{IF ROW_COUNT > 100M: -
**
⚠️ WARNING:
**
Large table - always add date filters}
##
Relationships
...
# Scripts are relative to ../analyzing-data/
uv run cli.py concept
import
-p
.astro/warehouse.md
uv run cli.py concept learn customers HQ.MART_CUST.CURRENT_ASTRO_CUSTS
-k
ACCT_ID
...
(Recommended) - Append Quick Reference section
No, skip
- Use warehouse.md and cache only
If user chooses Yes:
Check if
.claude/CLAUDE.md
or
...
**
Large tables (always filter by date):
**
{list tables with >100M rows}
>
Auto-generated by
`/data:init`
. Run
...
Weekly
: As a good practice, even if no known changes
