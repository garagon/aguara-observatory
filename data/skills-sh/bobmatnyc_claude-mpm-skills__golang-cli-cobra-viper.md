...
🎯
Cobra Commands
: POSIX-compliant CLI with subcommands (
app verb noun --flag
)
⚙️
Viper Config
...
🔄
Integration
: Seamless Cobra + Viper plumbing patterns
🐚
Shell Completion
: Auto-generated completions for bash, zsh, fish, PowerShell
✅
Production Ready
: Battle-tested by kubectl, docker, gh, hugo
Used By
: Kubernetes (kubectl), Docker CLI, GitHub CLI (gh), Hugo, Helm, and 100+ major projects
...
Long
:
`MyApp is a CLI tool that demonstrates best practices
...
Errorf
(
"production deploys require --force flag"
)
}
return
nil
...
Printf
(
"Deploying to %s in region %s (force=%v)\n"
,
env
,
region
...
// MYAPP_PORT, MYAPP_DATABASE_HOST, etc.
...
Manual mapping
for non-standard env var names:
viper
.
BindEnv
(
...
Errorf
(
"API key not configured (set MYAPP_API_KEY or add to config file)"
)
}
...
Errorf
(
"failed to initialize API client: %w"
,
err
)
}
...
Long
:
`Generate shell completion script for myapp.
To load completions:
Bash:
...
Errorf
(
"unknown format: %s (use json or table)"
,
format
)
}
...
name
:
"deploy to production without force"
,
args
:
[
...
// Respects: flag > env > config > default
...
// Always returns config file value
}
Correct
:
rootCmd
...
cmd/root.go
: See "Command Structure Pattern" section above
Building and installing
:
# Development
go run main.go deploy staging
--region
us-west-2
# Production build
go build
