...
enabled
(required): Boolean to activate/deactivate
true
: Rule is active
false
...
event
(required): Which hook event to trigger on
bash
: Bash tool commands
file
...
: All events
action
(optional): What to do when rule matches
warn
...
warn
pattern
(simple format): Regex pattern to match
...
python3
-c
"import re; print(re.search(r'your_pattern', 'test text'))"
...
:
log
# Matches "log", "login", "dialog", "catalog"
Better:
console\.log\(|logger\.
Too specific:
pattern
