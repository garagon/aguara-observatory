...
In Strapi v5,
always use the Document Service API
(
strapi.documents
) for all data operations. The Entity Service API from v4 is deprecated.
...
Publish
strapi.documents(uid).publish({ documentId })
N/A
Unpublish
strapi.documents(uid).unpublish({ documentId })
N/A
...
Run
npm run build
in plugin, check for import errors
...
for admin panel data fetching and mutations.
...
path
:
'/tasks/related/:relatedType/:relatedId'
,
handler
:
'task.findRelatedTasks'
,
}
,
]
...
Use
unstable_useContentManagerContext()
for current entity info
Use
app.getPlugin('content-manager').injectComponent()
for CM integration
Support translations with
registerTrads()
Content Types:
Use
morphToMany
...
91
Repository
ayhid/claude-skill-strapi-expert
First Seen
Jan 24, 2026
Security Audits
Gen Agent Trust Hub
