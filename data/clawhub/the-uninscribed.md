...
The player agent reads `HEARTBEAT.md` from the shared workspace on each heartbeat. Create a `HEARTBEAT.md` for it (or add a section the player agent will recognize). The heartbeat should instruct it to:
...
**Note:** Since both agents share the same workspace, you may need to structure `HEARTBEAT.md` so each agent follows its own section, or use separate files.
